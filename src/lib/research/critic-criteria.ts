...
export const CRITIC_CRITERIA: Record<ExecutableStage, StageCriteria> = {
  plan: {
    criteria: [
      ...
      "Measurable success criteria: quantitative, decidable pass/fail thresholds tied to the stated metrics, appropriate to the design — not only a headline effect threshold. E.g. for a benchmark/dataset contribution, manipulation-validity and task-competence thresholds; for a method/model/algorithm contribution, the fair-comparison conditions and the baseline numbers it must beat; for any other contribution type, the analogous decidable thresholds for its central claim.",
      "Valid-comparison / internal-validity protocol (matched to the contribution type — apply whatever rules out the most likely confound for THIS study's design, specified BEFORE the full run). E.g. for a benchmark/dataset contribution: gold-answer preservation across control/conflict surfaces, conflict-surface lure salience, control-surface absence of the same lure cue, per-group answer/lure label balance, and representative item-pair examples per family. For a method/model/algorithm contribution: matched training data, compute, and hyperparameter-tuning budget across all arms; no train/test or pretraining leakage into the evaluation; baselines configured to reproduce their known published numbers; and ablations that isolate the specific mechanism the gain is attributed to. For any other contribution type (e.g. a theory, probing, interpretability, observational, data/corpus, or systems/efficiency study), require the equivalent confound controls for its central comparison.",
      "Interpretability / competence floor (matched to the contribution type): the plan preregisters a floor that must be cleared before the headline effect is interpreted. E.g. for a benchmark/dataset, the evaluated (or a declared anchor) model must exceed BOTH random-choice and majority-class baselines on the control condition by a preregistered margin; for a method/model, the baseline arms must be competent — reproducing their known performance rather than crippled — so a gain cannot be an artifact of a weak baseline. For any other contribution type, require the analogous floor: a sane reference level below which the headline interpretation is not licensed. If the floor fails, the study reports a construction/feasibility result, not the headline claim.",
  ...
  experiment: {
    criteria: [
      ...
      "Valid comparison / internal validity (matched to the contribution type — rule out the most likely confound for THIS study's design and save the supporting artifact). E.g. for a benchmark/dataset contribution (paired prompts, transformed items, lures, or annotations): item-level validation proving control and conflict surfaces preserve the gold answer, conflict surfaces cue the preregistered lure, controls do not already cue the same lure, and answer/lure labels are balanced within family — spot-check at least 10 random item pairs. For a method/model/algorithm contribution: evidence that arms were matched on training data, compute, and hyperparameter-tuning budget; that there is no train/test or pretraining leakage into the evaluation; that baselines reproduce their known numbers; and ablations that isolate the claimed mechanism. For any other contribution type, the equivalent confound control for its central comparison. Fail if the relevant confound control is ambiguous, degenerate, or merely asserted.",
  ...
  analysis: {
    criteria: [
      ...
      "Measurement validity (matched to the contribution type): before hypothesis testing, the primary measurement is validated. E.g. for a scored/parsed benchmark: audit the parser/scorer against the raw outputs — parse-method distribution, invalid/unmatched rates, row-level changes vs upstream scoring, and an independent adjudication sample stratified across gold-correct, lure-error, non-lure-wrong, unmatched-text, and parser-changed rows. For a method/model contribution: confirm the metric is computed on the correct, uncontaminated split with the right denominator and protocol and no metric leakage. For any other contribution type, validate that the primary measurement actually measures the construct it claims. If plausible measurement error could move any headline result by more than ~2 percentage points, mark the affected claims inconclusive.",
      "Non-degenerate effect metric: a claimed effect is reported as an EXCESS over the appropriate base rate or reference, never a quantity that is mechanically guaranteed. E.g. for a lure-error / strategy-misselection claim, lure selection must EXCEED the matched-control and chance wrong-answer base rates (binary/two-choice families where 'lure among incorrect' is mechanically just 'wrong answer' are excluded from the confirmatory claim or analyzed separately). For a method gain, the improvement must exceed seed-to-seed variance against a competently-tuned baseline, not a single lucky run. For any other contribution type, the effect must exceed the natural null/baseline for its design.",
      "Claims supported AND interpretable: every stated finding is backed by the experiment's actual results (cross-check UPSTREAM_experiment.json) and no claim exceeds the data. The contribution's interpretability floor must hold: e.g. for a benchmark, if control-surface task competence is at or below random-choice/majority-class baselines, reject any strategy-level interpretation and limit the framing to benchmark-construction / model-incompetence; for a method, if a baseline arm is crippled or below its known performance, reject any 'our method wins' interpretation until a competent baseline is restored. For any other contribution type, reject interpretations not licensed once its analogous floor fails.",
  ...
  paper: {
    criteria: [
      ...
      "Method is reproducible AND auditable from the text: a reader could re-run the study AND inspect whether the central comparison is valid — path-heavy artifact references alone are NOT sufficient. Every contribution includes an artifact/release card stating exactly what is released and how to reproduce the headline numbers. E.g. for a benchmark/dataset contribution, ALSO include representative control/conflict item-pair examples per family (with gold and lure), a qualitative error table with real model outputs (correct, lure-error, non-lure-wrong, parser-failure), and a benchmark-validation summary (semantic-equivalence, lure-salience, label-balance, scoring-adjudication). For a method/model/algorithm contribution, ALSO report the exact training data, splits, compute budget, and full hyperparameters for every arm, evidence of no train/test leakage, baseline numbers shown to match known results, and the ablations isolating the claimed mechanism. For any other contribution type, ALSO include the equivalent reproducibility and validity evidence for its central claim.",
